...
static cl::opt<bool> RunLinear("x-linear", cl::init(false),
                               cl::desc("OLLVM - linearize "));
...
struct LinearizeX {
  ...
  std::pair<size_t, Align> getDiscardSizeAlign(Function &F,
  ...
  Value *prepareBlock(BasicBlock &bb, Value *on, AllocaInst *selector,
                      label label, AllocaInst *discard);
...
bool translateMemset(CallInst *ci) {
  ...
  Builder.CreateCall(
      fc, {ci->getOperand(0),
           Builder.CreateZExt(ci->getOperand(1), Builder.getInt32Ty()),
...
static std::unordered_map<Intrinsic::ID, std::function<bool(CallInst *)>>
    intrinMap = {{Intrinsic::lifetime_start, removeIntrin},
                 {Intrinsic::lifetime_end, removeIntrin},
                 {Intrinsic::memset, translateMemset},
                 {Intrinsic::memcpy, translateMemcpy}};

void LinearizeX::handleIntrinsics(BasicBlock &BB) {
...
void LinearizeX::resolveContinuityErrors(Function &F) {
  ...
  for (Instruction *ins : tofix) {
    ...
    AllocaInst *alloc =
        new AllocaInst(ty, 0, Twine(ins->getName(), ".ins.rewrite"), first_ins);
 ...
 */
void LinearizeX::eliminatePhis(Function &F) {
  ...
  for (PHINode *phi : phis) {
    ...
    for (unsigned i = 0; i < phi->getNumIncomingValues(); ++i) {
      ...
      if (phi->getBasicBlockIndex(bb) != i)
        continue;
...
void LinearizeX::shuffleBlocks(Function &F, size_t numlbl) {
...
size_t LinearizeX::getLabels(Function &F) {
...
template <>
void LinearizeX::linearizeIns<LoadInst>(LoadInst *ins, Value *on,
                                        AllocaInst *discard) {
  ...
  *ins->op_begin() = Builder.CreateSelect(
      on, ins->getPointerOperand(),
      Builder.CreatePointerCast(discard, ins->getPointerOperandType()));
}

template <>
void LinearizeX::linearizeIns<StoreInst>(StoreInst *ins, Value *on,
                                         AllocaInst *discard) {
  IRBuilder<> Builder(ins);
...
template <>
void LinearizeX::linearizeIns<AtomicCmpXchgInst>(AtomicCmpXchgInst *ins,
                                                 Value *on,
                                                 AllocaInst *discard) {
  IRBuilder<> Builder(ins);
  *ins->op_begin() = Builder.CreateSelect(
...
template <>
void LinearizeX::linearizeIns<AtomicRMWInst>(AtomicRMWInst *ins, Value *on,
                                             AllocaInst *discard) {
  IRBuilder<> Builder(ins);
  *ins->op_begin() = Builder.CreateSelect(
      on, ins->getPointerOperand(),
...
template <>
void LinearizeX::linearizeIns<CallInst>(CallInst *ins, Value *on,
                                        AllocaInst *discard) {
  IRBuilder<> Builder(ins);
  ins->setCalledOperand(Builder.CreateSelect(
      on, ins->getCalledOperand(),
      Builder.CreatePointerCast(NopFun, ins->getCalledOperand()->getType())));
}

template <>
void LinearizeX::linearizeIns<GetElementPtrInst>(GetElementPtrInst *ins,
                                                 Value *on,
                                                 AllocaInst *discard) {
...
template <>
void LinearizeX::linearizeIns<BinaryOperator>(BinaryOperator *op, Value *on,
                                              AllocaInst *discard) {
  ...
  case Instruction::LShr:
    ...
    op->setOperand(1,
                   Builder.CreateAnd(
                       op->getOperand(1),
                       ConstantInt::get(typ, typ->getIntegerBitWidth() - 1)));
    break;
  default:
    break;
  }
...
Value *LinearizeX::prepareBlock(BasicBlock &bb, Value *on, AllocaInst *selector,
                                label label, AllocaInst *discard) {
  ...
  for (Instruction &ins : bb) {
    ...
    if (isa<LoadInst>(&ins) || isa<StoreInst>(&ins) ||
        isa<AtomicCmpXchgInst>(&ins) || isa<AtomicRMWInst>(&ins) ||
...
std::pair<size_t, Align>
LinearizeX::getDiscardSizeAlign(Function &F, const DataLayout &DataLayout) {
  ...
  for (BasicBlock &bb : F) {
    for (Instruction &ins : bb) {
      ...
      if (si) {
        ...
        Align tmpalign =
            DataLayout.getValueOrABITypeAlignment(si->getAlign(), t);
        if (tmp > size)
          size = tmp;
        if (tmpalign > align)
          align = tmpalign;
      ...
      if (cmpXchg) {
        ...
        size_t tmp = DataLayout.getTypeAllocSize(t);
        Align tmpalign =
            DataLayout.getValueOrABITypeAlignment(cmpXchg->getAlign(), t);
        if (tmp > size)
          size = tmp;
        if (tmpalign > align)
          align = tmpalign;
...
bool LinearizeX::runOnFunction(Function &F) {
  ...
  while (isa<AllocaInst>(it) ||
         (isa<CallInst>(it) &&
          cast<CallInst>(it)->getIntrinsicID() != Intrinsic::not_intrinsic) ||
         isa<StoreInst>(it)) {
    ++it;
  }
  ...
  std::pair<size_t, Align> discardSizeAlign =
      getDiscardSizeAlign(F, F.getParent()->getDataLayout());
  ...
  AllocaInst *retval;
  {
    ...
    discard = Builder.CreateAlloca(
        ArrayType::get(Builder.getInt8Ty(), discardSizeAlign.first), nullptr,
        "discard");
  ...
  for (auto bbit = ++F.begin(); bbit != F.end(); ++bbit) {
    ...
    if (br) {
      ...
      if (br->isConditional()) {
        ...
        if (tl.kind != LBL_FALLTHROUGH && fl.kind != LBL_FALLTHROUGH) {
          Builder.CreateStore(
              Builder.CreateSelect(br->getCondition(),
                                   Builder.getInt64(tl.fresh),
                                   Builder.getInt64(fl.fresh)),
              Builder.CreateSelect(
                  on, selector,
                  Builder.CreatePointerCast(
                      discard, Builder.getInt64Ty()->getPointerTo())));
          continue;
        }
        Value *ptr;
        size_t val;
        if (tl.kind == LBL_FRESH) {
          ptr = Builder.CreateSelect(
              Builder.CreateAnd(br->getCondition(), on), selector,
              Builder.CreatePointerCast(discard,
                                        Builder.getInt64Ty()->getPointerTo()));
          val = tl.fresh;
          on = Builder.CreateAnd(on, Builder.CreateNot(br->getCondition()),
                                 Twine(br->getSuccessor(1)->getName(), ".on"));
        } else {
          assert(fl.kind == LBL_FRESH);
          ptr = Builder.CreateSelect(
              Builder.CreateOr(br->getCondition(), Builder.CreateNot(on)),
              Builder.CreatePointerCast(discard,
                                        Builder.getInt64Ty()->getPointerTo()),
              selector);
          val = fl.fresh;
          on = Builder.CreateAnd(on, br->getCondition(),
                                 Twine(br->getSuccessor(0)->getName(), ".on"));
        }
    ...
    if (rv) {
      Builder.CreateStore(rv, Builder.CreateSelect(
                                  on, retval,
                                  Builder.CreatePointerCast(
                                      discard, rv->getType()->getPointerTo())));
    }
    Builder.CreateStore(
  ...
  BasicBlock *commonFin =
      BasicBlock::Create(F.getParent()->getContext(), "fin", &F);
  {
  ...
  // merge bbs
  {
    ...
    BasicBlock *tar = BasicBlock::Create(F.getParent()->getContext(), "merged",
                                         &F, &*std::next(F.begin()));

    // rewire branch from entry block
    new StoreInst(
        ConstantInt::get(IntegerType::get(F.getParent()->getContext(), 64),
                         labels[F.begin()->getSingleSuccessor()].fresh, false),
        selector, F.begin()->getTerminator());
    cast<BranchInst>(F.begin()->getTerminator())->setSuccessor(0, tar);

    std::list<BasicBlock *> bblist;
    size_t num_inst = 0;
    for (auto it = std::next(std::next(F.begin())); std::next(it) != F.end();
         ++it) {
    ...
    */
    ...
    // this loop interleaves the instructions from all basic blocks to make it
    // more difficult to separate basic blocks. Unfortunately this reintroduces
    // jumps (on x86) as the register preassure is significantly higher. If you
    ...
    while (num_inst) {
      ...
      /*
      for (size_t pos = (*rng)() % num_inst; pos > (*it)->size(); pos -=
      (*it)->size()) {
              ++it;
      }*/
...
PreservedAnalyses Linearize::run(Module &M, ModuleAnalysisManager &MAM) {
  ...
  LinearizeX x;
  x.rng = M.createRNG(rngstr);

  x.NopFun = Function::Create(
      FunctionType::get(IntegerType::get(M.getContext(), 8)->getPointerTo(), {},
                        true),
      GlobalValue::LinkOnceODRLinkage, "__obf_nop", M);
  ...
  {
    BasicBlock *bb = BasicBlock::Create(M.getContext(), "entry", x.NopFun);
    ReturnInst::Create(M.getContext(),
                       Constant::getNullValue(x.NopFun->getReturnType()), bb);
  }
